--[[
    ...
    Usando Linity UI (A MAIS AVANÇADA)
...
local Window = Linity:CreateWindow({
    Name = "🔥 Nameless Admin",
    Subtitle = "por CriadorYan",
...
local function changeBackground(imageId)
    if not imageId or imageId == "" then
        notify("Erro", "Link da imagem inválido!")
        return
    end
...
local function enableInvisible()
    ...
    notify("Sucesso", "Invisibilidade ativada! NINGUÉM te vê", 3)
end
...
local function enableCameraNoclip()
    ...
    notify("Sucesso", "Câmera atravessa paredes!", 3)
end
...
local function disableCameraNoclip()
    ...
    notify("Sucesso", "Câmera restaurada!", 2)
end
                ...
                if otherRoot and otherHum then
                    ...
                    if distance < 50 then
                        ...
                        flingForce.Velocity = Vector3.new(
                            direction.X * 50000 + math.random(-20000, 20000),
                            50000 + math.random(0, 30000),
                            direction.Z * 50000 + math.random(-20000, 20000)
                        )
                        ...
                        otherRoot.Velocity = Vector3.new(
                            direction.X * 500 + math.random(-200, 200),
                            500 + math.random(0, 300),
                            direction.Z * 500 + math.random(-200, 200)
                        )
...
local function destroyHub()
    ...
    if playerGui then
        for _, gui in pairs(playerGui:GetChildren()) do
            if gui.Name:find("Linity") or gui.Name:find("Rayfield") or gui.Name == "NamelessBackground" then
                gui:Destroy()
            end
        end
    end
...
local function enableAirSwim()
    ...
    local function createSwimEffect()
        ...
        if hum and root then
            ...
            if not sv then
                sv = Instance.new("BodyVelocity")
                sv.Name = "SwimVelocity"
                sv.MaxForce = Vector3.new(5000, 5000, 5000)
                sv.Parent = root
            end
            local md = hum.MoveDirection
            ...
            if md.Magnitude > 0 then
                ...
                sv.Velocity = Vector3.new(0, 0.5, 0)
            end
        end
    end
...
local function morphPlayer(targetPlayer)
    ...
    notify("Sucesso", "Transformado em: " .. targetPlayer.Name, 3)
end
    ...
    for _, plr in pairs(game.Players:GetPlayers()) do
        if plr ~= player then
            ...
            plr.CharacterAdded:Connect(function(char)
                if espEnabled then task.wait(0.5); createESP(char, plr.Name, Color3.fromRGB(0, 255, 255), false) end
            end)
        end
    end
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and not game.Players:GetPlayerFromCharacter(obj) then
            if obj.Humanoid.Health > 0 then
                createESP(obj, "[NPC] " .. obj.Name, Color3.fromRGB(255, 255, 0), true)
            end
        end
    end
...
MainTab:CreateSlider({
    ...
    Callback = function(Value)
        local hum = getHumanoid()
        if hum then hum.JumpPower = Value; hum.UseJumpPower = true end
    end,
})

MainTab:CreateSection("📋 Informações")
MainTab:CreateLabel("🔥 Nameless Admin por CriadorYan")
MainTab:CreateLabel("📱 Otimizado para Mobile")
...
FlyTab:CreateToggle({
    ...
    CurrentValue = false,
    Callback = function(Value)
        if Value then startFly() else if flying then stopFly() end end
    end,
})
...
FlyTab:CreateSection("📱 Controles")
FlyTab:CreateLabel("🎮 Joystick frente = Voar frente")
FlyTab:CreateLabel("⬆️ Olhar cima + frente = Subir")
FlyTab:CreateLabel("⬇️ Olhar baixo + frente = Descer")
...
MovementTab:CreateToggle({
    Name = "Air Walk",
    CurrentValue = false,
    Callback = function(v) if v then enableAirWalk() else disableAirWalk() end end,
})

MovementTab:CreateToggle({
    Name = "Air Swim",
    CurrentValue = false,
    Callback = function(v) if v then enableAirSwim() else disableAirSwim() end end,
})

MovementTab:CreateToggle({
    Name = "Infinite Jump",
    CurrentValue = false,
    Callback = function(v) if v then enableInfiniteJump() else disableInfiniteJump() end end,
})
...
CameraTab:CreateToggle({
    Name = "Camera Noclip",
    CurrentValue = false,
    Callback = function(v) if v then enableCameraNoclip() else disableCameraNoclip() end end,
})

CameraTab:CreateToggle({
    Name = "Zoom Infinito",
    CurrentValue = false,
    Callback = function(v) if v then enableInfiniteCamera() else disableInfiniteCamera() end end,
})

CameraTab:CreateSection("📋 Info")
CameraTab:CreateLabel("📷 Camera Noclip: Atravessa paredes")
CameraTab:CreateLabel("🔍 Zoom Infinito: Afaste até 999k studs")
...
WorldTab:CreateToggle({
    Name = "NoFog",
    CurrentValue = false,
    Callback = function(v) if v then enableNoFog() else disableNoFog() end end,
})
...
TrollTab:CreateToggle({
    Name = "Fling TURBINADO",
    CurrentValue = false,
    Callback = function(v) if v then enableFling() else disableFling() end end,
})
...
for _, tp in pairs(game.Players:GetPlayers()) do
    if tp ~= player then
        TrollTab:CreateButton({
            Name = "Morph: " .. tp.Name,
            Callback = function() morphPlayer(tp) end,
        })
    end
end
...
game.Players.PlayerAdded:Connect(function(np)
    if np ~= player then
        TrollTab:CreateButton({
            Name = "Morph: " .. np.Name,
            Callback = function() morphPlayer(np) end,
        })
    end
end)
...
VisualTab:CreateToggle({
    Name = "Invisível (NINGUÉM te vê)",
    CurrentValue = false,
    Callback = function(v) if v then enableInvisible() else disableInvisible() end end,
})

VisualTab:CreateLabel("✅ Transparência total forçada")
VisualTab:CreateLabel("✅ Loop anti-render constante")
VisualTab:CreateLabel("✅ Funciona ao renascer")
...
VisualTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            local function ah(c)
                if c and not c:FindFirstChild("PlayerHighlight") then
            ...
            for _, plr in pairs(game.Players:GetPlayers()) do
                if plr ~= player and plr.Character then ah(plr.Character) end
            end
        else
            for _, plr in pairs(game.Players:GetPlayers()) do
...
ConfigTab:CreateSection("🖼️ Background")
ConfigTab:CreateLabel("Cole o link da imagem abaixo:")

ConfigTab:CreateTextbox({
    Name = "Link da Imagem",
    PlaceholderText = "URL ou ID do Roblox...",
...
ConfigTab:CreateButton({
    ...
    Callback = function() removeBackground() end,
})
...
ConfigTab:CreateLabel("⚠️ Ao destruir, tudo será removido!")
